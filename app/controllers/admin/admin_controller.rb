class Admin::AdminController < ApplicationController
  ...
  def index
    ...
    @today_visits           = Visit.where('started_at > :date', { date: Date.today - 1.day }).count
    @today_views            = Ahoy::Event.where(name: '$view').where('time > :date', { date: Date.today.beginning_of_day }).count
