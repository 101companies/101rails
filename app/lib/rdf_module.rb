module RdfModule
  ...
  def page_to_resource(title)
    ...
    "#{page.namespace}:#{page.title.tr(' ', '_')}"
  end
  ...
  def get_rdf_graph(title, directions)
    ...
    uri = page_to_resource(title)
    graph = []
    graph = add_outgoing_semantic_triples(graph, @page, uri, directions)
    graph = add_outgoing_non_semantic_triples(graph, uri, directions) unless directions
  ...
  def add_outgoing_non_semantic_triples(graph, uri, directions)
    (@page.internal_links - @page.semantic_links).each do |link|
      ...
      graph << [uri, 'mentions', object] unless object.nil?
    end
    graph
  end
    ...
    if page.subresources.nil?
      ...
      page.subresources.each do |s|
        s.each do |key, value|
          ...
          value.each do |l|
            predicate = RDF::URI.new(semantic_properties(l.split('::')[0]))
  ...
  def add_outgoing_semantic_triples(graph, page, uri, directions)
    page.semantic_links.each do |link|
      ...
      graph << [subject, semantic_property, object] unless object.nil?
    end
  ...
  def add_ingoing_triples(graph, page)
    ...
    titles = [page.full_title, page.full_title.tr(' ', '_'), page.full_title.tr('_', ' ')]
